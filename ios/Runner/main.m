...
int darksword_exploit_entry(int argc, char* argv[])
{
	...
	// Tìm kiếm và khởi tạo bộ nhớ tiến trình FreeFire
	// Lưu ý: offset_allproc cần được điều chỉnh chính xác với kernel_base cho phiên bản iOS cụ thể
	uint64_t allproc_addr = kernel_base + 0x1B38000; // Offset ví dụ cho allproc
	uint64_t kern_proc_head = early_kread64(allproc_addr);

	TargetProcess game_proc;
	if (find_process_by_name("FreeFire", kern_proc_head, &game_proc)) {
		printf("[+] Đã gắn thành công vào bộ nhớ tiến trình Free Fire (PID: %d)!\n", game_proc.pid);
	}

	// Gọi hàm test đọc bộ nhớ để in log chi tiết
